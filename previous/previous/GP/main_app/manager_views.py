...
def update_attendance(request):
    ...
    # Debug logging of request parameters
    print(f"UPDATE Attendance Request - Date: {attendance_date}, Department ID: {department_id}")
    print(f"Employee IDs JSON length: {len(employee_ids_json) if employee_ids_json else 'None'}")
    ...
    try:
        ...
        print(f"Department: {department_id}, Date: {attendance_date}, Employee count: {len(employee_ids)}")
        print(f"Employee data sample: {employee_ids[:2] if employee_ids else 'None'}")
        ...
        for employee_data in employee_ids:
            try:
                # Validate employee data structure
                if not isinstance(employee_data, dict) or 'id' not in employee_data or 'status' not in employee_data:
                    error_msg = f"Invalid employee data format: {employee_data}"
                    print(error_msg)
                    employee_errors.append(error_msg)
                    continue
                ...
                try:
                    ...
                    employee_id = str(employee_data['id']).strip()  # Fallback to string
                
                # Get status value and convert to boolean
                status_value = int(employee_data.get('status', 0))
                status = status_value == 1
                
                print(f"Processing employee ID: {employee_id}, Status: {status}")
                ...
                try:
                    ...
                    if created:
                        created_count += 1
                        print(f"Created new attendance report for employee {employee_id} with status {status}")
                    else:
                        # Only update if the status is different
                        if attendance_report.status != status:
                            attendance_report.status = status
                            attendance_report.save()
                            print(f"Updated status to {status} for employee {employee_id} (was {not status})")
                        else:
                            print(f"Status already {status} for employee {employee_id}, no update needed")
                ...
                error_msg = f"Error processing employee {employee_data.get('id', 'unknown')}: {str(e)}"
                print(error_msg)
                employee_errors.append(error_msg)
        
        if updated_count > 0:
            success_msg = f"Successfully updated attendance for {updated_count} employees ({created_count} new records)"
            if employee_errors:
                success_msg += f" with {len(employee_errors)} errors"
            print(success_msg)
            return JsonResponse({
                "success": True, 
                "message": success_msg,
                ...
                "error_count": len(employee_errors)
            })
        else:
            error_msg = "No attendance records were updated. Errors: " + "; ".join(employee_errors[:3])
            if len(employee_errors) > 3:
                error_msg += f" and {len(employee_errors) - 3} more errors"
            print(error_msg)
            return JsonResponse({
                "success": False, 
                "message": error_msg,
                "errors": employee_errors[:10] if employee_errors else []
            }, status=400)
